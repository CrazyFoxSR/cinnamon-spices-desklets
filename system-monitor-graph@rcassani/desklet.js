...
// import libary for net monitoring
...
SystemMonitorGraph.prototype = {
    ...
    _init: function(metadata, desklet_id) {
        ...
        this.settings.bindProperty(Settings.BindingDirection.IN, "net-direction", "net_direction", this.on_setting_changed);
        this.settings.bindProperty(Settings.BindingDirection.IN, "net-speed-scale", "net_speed_scale", this.on_setting_changed);
        this.settings.bindProperty(Settings.BindingDirection.IN, "net-device", "net_device", this.on_setting_changed);
    ...
    update_draw: function() {
        ...
        if (this.first_run){
            ...
            this.hdd_cpu_tot = 0;
            this.hdd_hdd_tot = 0;
        //  values needed for net monitoring
            this.net_upload = 0;
            this.net_download = 0;
        ...
        // set libary for net monitoring
        let devices = GTop.glibtop_get_netlist(new GTop.glibtop_netlist());
        let gtop = new GTop.glibtop_netload();
        let device_index=this.net_device;
        if (devices[device_index]==null){
            device_index=0
        }
        GTop.glibtop_get_netload(gtop,devices[device_index]);
        // current values
        switch (this.type) {
          case "cpu":
          ...
          case "gpu":
              ...
              break;
              case  "net":
                  switch(this.net_direction){
                      case "upload":
                        let net_upload_data = this.get_net_upload(gtop);
                        let net_upload_value= net_upload_data/this.net_speed_scale;
                        net_upload_value= (net_upload_value>100)?100:net_upload_value;
                        value=net_upload_value/100;
                        text1="Upload";
                        text2=Math.round(net_upload_value).toString()+" %";
                        text3=devices[device_index];
                         break;
                        
                      case "download":
                        let net_download_data = this.get_net_download(gtop);
                        let net_download_value= net_download_data/this.net_speed_scale;
                        net_download_value=(net_download_value>100)?100:net_download_value;
                        value=net_download_value/100;
                        text1="Download";
                        text2=Math.round(net_download_value).toString()+" %";;
                        text3=devices[device_index];
                          break;
                  }
    ...
    get_net_upload: function(gtop){
        let refresh_time = this.refresh_interval;
        let net_upload_new =0
        let net_upload_tot=0
        
        net_upload_new = gtop.bytes_out;
        net_upload_tot= Math.round((net_upload_new - this.net_upload)/refresh_time);
        this.net_upload = net_upload_new;
      
       
        return net_upload_tot;



    },
    get_net_download: function(gtop){
        let refresh_time = this.refresh_interval;
        // let net_download_tot = 5;
        let net_download_new = gtop.bytes_in;
        let net_download_tot= Math.round((net_download_new- this.net_download)/refresh_time);
        this.net_download = net_download_new;
       
        return net_download_tot;
    },
};
